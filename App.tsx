...
const MusicPlayer: React.FC = () => {
  ...
  useEffect(() => {
    // Construct the audio path based on environment
    ...
    const handleError = (e: Event) => {
      console.error("Audio load error:", audioPath, e);
      setError(true);
      setLoaded(false);
    };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
        ...
        className={`p-4 rounded-full border backdrop-blur-xl transition-all duration-300 shadow-2xl group relative overflow-hidden ${
          error ? 'bg-red-900/40 border-red-400 text-red-300 cursor-not-allowed' :
          !loaded ? 'bg-gray-900/40 border-gray-400 text-gray-400 cursor-wait' :
          playing ? 'bg-cyan-900/40 border-cyan-400 text-cyan-300' : 
          'bg-white/5 border-white/20 text-gray-400 hover:border-cyan-400/50'
        }`}
        title={error ? 'Audio file error - check console' : !loaded ? 'Loading audio...' : playing ? 'Pause music' : 'Play music (Click to start)'}
        >
        <div className={`absolute inset-0 bg-gradient-to-r from-cyan-500/20 to-purple-500/20 opacity-0 group-hover:opacity-100 transition-opacity ${!loaded || error ? 'hidden' : ''}`}></div>
  ...
  return (
    ...
    >
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <button onClick={handleBack} className="inline-flex items-center text-gray-400 hover:text-cyan-400 mb-12 transition-all gap-3 group px-5 py-3 hover:bg-gradient-to-r hover:from-cyan-900/20 hover:to-purple-900/20 rounded-full cursor-pointer border border-transparent hover:border-cyan-500/30">
          <IconArrowLeft />
